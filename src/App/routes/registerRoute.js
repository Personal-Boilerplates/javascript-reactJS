...
export class RegisterRouterObject {
    ...
    constructor ({path, component, Layout, subRoutes, exact}) {
        ...
        const validPath = Array.isArray(path) ? path.some(p => notEmptyString(p)) : notEmptyString(path);

        if (validPath || typeof component !== "function") {
            throw new Error("You must inform a path and component to register a route");
        } else if (subRoutes && subRoutes.some(e => !(e instanceof RegisterRouterObject))) {
            throw new Error("subRoute type must be instance of RegisterRouterObject or undefined");
        }
        lastKey++;
